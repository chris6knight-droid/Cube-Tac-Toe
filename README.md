...
A browser game that fuses a 3×3 Rubik's Cube with Tic Tac Toe. You (**X**) play against an AI bot (**O**). Built with [Three.js](https://threejs.org/) in a single `index.html`.
...
2. On your turn (you are **X** and go first):
   - **Place your mark** — tap any empty sticker. Marks have a white outline so they stay readable on every face color.
...
3. The **bot (O)** then takes its turn automatically — it places a mark and makes a move.
4. **Win** by getting 3 of your marks in a row — horizontally, vertically, or diagonally — on a single 3×3 face. The win is checked **only after the rotation finishes**, never the instant a mark is placed.
